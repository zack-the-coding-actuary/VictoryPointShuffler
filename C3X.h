...
struct c3x_config {
	...
	// Victory points
	bool enable_victory_points;
	int  victory_points_per_player_percent;
};
...
struct injected_state {
	...
	// VP tile indices selected during place_victory_points, waiting to be shuffled into.
	// Applied either immediately (scenario/save loads) or deferred to patch_Leader_begin_turn
	// (random map games, where the engine auto-places VPs after map generation).
	int * pending_vp_tile_indices;
	int   pending_vp_count;
	bool  vps_await_shuffle; // true = pending positions chosen, waiting for engine VP auto-place to finish
