...
void place_victory_points (void);
static void shuffle_victory_points (void);
	...
	if (success) {
		...
		if ((tile == NULL) || (tile == p_null_tile)) {
			...
			if ((district_id < 0) || (district_id >= is->district_count)) {
				char msg[200];
				snprintf (msg, sizeof msg, "district (unrecognized name: \"%s\")", entry->district_name);
				...
				if (inst == NULL) {
					...
					if (district_id == WONDER_DISTRICT_ID) {
						...
						if (! has_city || ! has_wonder) {
							...
							if (wonder_index < 0) {
								char msg[200];
								snprintf (msg, sizeof msg, "wonder_name (unrecognized wonder: \"%s\")", entry->wonder_name);
								add_scenario_district_error (parse_errors, section_start_line, msg);
								success = 0;
							} else {
								inst->wonder_info.city = NULL;
						...
						if (! has_name) {
							...
							if (natural_index < 0) {
								char msg[200];
								snprintf (msg, sizeof msg, "wonder_name (unrecognized natural wonder: \"%s\")", entry->wonder_name);
								add_scenario_district_error (parse_errors, section_start_line, msg);
								success = 0;
							} else
...
{
	char * scenario_filename = "scenario.districts.txt";
	char * scenario_districts_path = BIC_get_asset_path (p_bic_data, __, scenario_filename, false);

	// BIC_get_asset_path returns the file name when it can't find the file
	if ((scenario_districts_path == NULL) || (0 == strcmp (scenario_filename, scenario_districts_path)))
		return;

	char * text = file_to_string (scenario_districts_path);
	if (text == NULL)
		return;
	...
	if ((parse_errors != NULL) || (unrecognized_keys != NULL)) {
		...
		popup->vtable->set_text_key_and_flags (popup, __, is->mod_script_path, "C3X_WARNING", -1, 0, 0, 0);
		char header[256];
		snprintf (header, sizeof header, "District scenario file issues in %s:", scenario_districts_path);
...
patch_init_floating_point ()
{
	...
	struct boolean_config_option {
		...
		{"enable_victory_points"                                 , false, offsetof (struct c3x_config, enable_victory_points)},
	...
	struct integer_config_option {
		...
		{"victory_points_per_player_percent"             ,    100, offsetof (struct c3x_config, victory_points_per_player_percent)},
	...
	is->pending_vp_tile_indices = NULL;
	is->pending_vp_count = 0;
	is->vps_await_shuffle = false;
	is->paused_for_popup = false;
...
bool
is_below_stack_limit (Tile * tile, int civ_id, int type_id)
{
	...
	if (itable_look_up_or (&is->current_config.exclude_types_from_units_per_tile_limit, type_id, 0))
		return true;
	...
	FOR_UNITS_ON (uti, tile) {
		...
		int uti_type_id = uti.unit->Body.UnitTypeID;
		if ((uti.unit->Body.Container_Unit < 0) &&
		    (class == p_bic_data->UnitTypes[uti_type_id].Unit_Class) &&
		    ! itable_look_up_or (&is->current_config.exclude_types_from_units_per_tile_limit, uti_type_id, 0)) {
			stack_limit -= 1;
			if (stack_limit <= 0)
				return false;
		}
...
patch_Unit_can_move_to_adjacent_tile (Unit * this, int edx, int neighbor_index, int param_2)
{
	...
	int type_id = this->Body.UnitTypeID;
	if ((base_validity == AMV_OK) && (is->current_config.limit_units_per_tile[p_bic_data->UnitTypes[type_id].Unit_Class] > 0)) {
		int nx, ny;
		get_neighbor_coords (&p_bic_data->Map, this->Body.X, this->Body.Y, neighbor_index, &nx, &ny);
		if (! is_below_stack_limit (tile_at (nx, ny), this->Body.CivID, type_id))
...
patch_Trade_Net_get_movement_cost (Trade_Net * this, int edx, int from_x, int from_y, int to_x, int to_y, Unit * unit, int civ_id, unsigned flags, int neighbor_index, Trade_Net_Distance_Info * dist_info)
{
	...
	// Apply unit count per tile limit
	if ((unit != NULL) && ! is_below_stack_limit (tile_at (to_x, to_y), unit->Body.CivID, unit->Body.UnitTypeID))
		return -1;
...
patch_Map_process_after_placing (Map * this, int edx, bool param_1)
{
	...
	// Try to shuffle VPs right after placing: if VP objects already
	// exist at this point, the renderer will see correct positions.
	if (is->vps_await_shuffle)
		shuffle_victory_points ();
}

static bool
vp_tile_is_valid (Tile * tile)
{
	if ((tile == NULL) || (tile == p_null_tile))
		return false;
	if (tile->vtable->m35_Check_Is_Water (tile))
		return false;
	enum SquareTypes sq = tile->vtable->m50_Get_Square_BaseType (tile);
	return sq != SQ_Mountains && sq != SQ_Volcano;
}


void
place_victory_points (void)
{
	int count   = p_bic_data->Map.Civ_Count;
	// Collect all valid candidate tiles (index + coordinates)
	int capacity = 256;
	int n_candidates = 0;
	int * cand_is = (int *)malloc (capacity * sizeof (int));
	int * cand_xs = (int *)malloc (capacity * sizeof (int));
	int * cand_ys = (int *)malloc (capacity * sizeof (int));
	if ((cand_is == NULL) || (cand_xs == NULL) || (cand_ys == NULL)) {
		free (cand_is); free (cand_xs); free (cand_ys);
		return;
	}
	...
	for (int i = 0; i < tile_count; i++) {
		int x, y;
		tile_index_to_coords (&p_bic_data->Map, i, &x, &y);
		Tile * tile = tile_at (x, y);
		if (! vp_tile_is_valid (tile))
			continue;
		if (n_candidates >= capacity) {
			int new_cap = capacity * 2;
			int * gi = (int *)realloc (cand_is, new_cap * sizeof (int));
			int * gx = (int *)realloc (cand_xs, new_cap * sizeof (int));
			int * gy = (int *)realloc (cand_ys, new_cap * sizeof (int));
			if ((gi == NULL) || (gx == NULL) || (gy == NULL)) { free (cand_is); free (cand_xs); free (cand_ys); return; }
			cand_is = gi; cand_xs = gx; cand_ys = gy;
			capacity = new_cap;
		}
		cand_is[n_candidates] = i;
		cand_xs[n_candidates] = x;
		cand_ys[n_candidates] = y;
		n_candidates++;
	}
	...
	// Try to place all VPs; retry with progressively relaxed min separation if needed.
	// Start at 2x Civ_Distance (or 16); step down by 2 each round; give up below 2.
	int min_sep_start = p_bic_data->Map.Civ_Distance > 0
	                  ? p_bic_data->Map.Civ_Distance * 2
	                  : 16;
	int max_vps = (count < 64) ? count : 64;
	int placed_is[64];
	int placed_xs[64];
	int placed_ys[64];
	int placed   = 0;
	...
	for (int min_sep = min_sep_start; min_sep >= 2 && !success; min_sep -= 2) {
		for (int attempt = 0; attempt < 100 && !success; attempt++) {
			// Fisher-Yates shuffle; vary seed by attempt so each try is independent
			int seed = (p_bic_data->Map.Seed ^ 0xC3771337) + attempt * 1234567;
			for (int i = n_candidates - 1; i > 0; i--) {
				seed = seed * 1664525 + 1013904223;
				int j = (int)((unsigned int)seed % (unsigned int)(i + 1));
				int ti = cand_is[i]; cand_is[i] = cand_is[j]; cand_is[j] = ti;
				int tx = cand_xs[i]; cand_xs[i] = cand_xs[j]; cand_xs[j] = tx;
				int ty = cand_ys[i]; cand_ys[i] = cand_ys[j]; cand_ys[j] = ty;
			}

			// Greedily pick tiles satisfying the current minimum separation
			placed = 0;
			for (int i = 0; (i < n_candidates) && (placed < max_vps); i++) {
				int x = cand_xs[i];
				int y = cand_ys[i];
				bool too_close = false;

				// Check distance against already-placed VPs
				for (int j = 0; j < placed; j++) {
					if (compute_wrapped_manhattan_distance (x, y, placed_xs[j], placed_ys[j]) < min_sep) {
						too_close = true;
						break;
					}
				}

				// Check distance against player starting locations
				if (! too_close) {
					for (int j = 1; j <= p_bic_data->Map.Civ_Count; j++) {
						int idx = p_bic_data->Map.Starting_Locations[j];
						if ((idx < 0) || (idx >= p_bic_data->Map.TileCount))
							continue;
						if (tile_at_index (&p_bic_data->Map, idx) == p_null_tile)
							continue;
						int sx, sy;
						tile_index_to_coords (&p_bic_data->Map, idx, &sx, &sy);
						if (compute_wrapped_manhattan_distance (x, y, sx, sy) < min_sep) {
							too_close = true;
							break;
						}
					}
	...
	if (! success)
		return; // Could not place all VPs at any separation; leave pending empty

	// Store placed tile indices for shuffle_victory_points() to apply later
	free (is->pending_vp_tile_indices);
	is->pending_vp_tile_indices = NULL;
	is->pending_vp_count = 0;
	is->pending_vp_tile_indices = (int *)malloc (placed * sizeof (int));
	if (is->pending_vp_tile_indices != NULL) {
		is->pending_vp_count = placed;
		for (int k = 0; k < placed; k++)
			is->pending_vp_tile_indices[k] = placed_is[k];
	}

}

// Appends msg to the VP debug log file in the Civ3Augments folder.
#define VP_DBG_LOG "Civ3Augments\\vp_shuffle_debug.txt"
static void
vp_dbg_log (char const * msg)
{
	typedef BOOL  (WINAPI * WriteFile_t)      (HANDLE, LPCVOID, DWORD, LPDWORD, LPOVERLAPPED);
	typedef DWORD (WINAPI * SetFilePointer_t) (HANDLE, LONG, PLONG, DWORD);
	WriteFile_t      wf  = (WriteFile_t)     (*p_GetProcAddress)(is->kernel32, "WriteFile");
	SetFilePointer_t sfp = (SetFilePointer_t)(*p_GetProcAddress)(is->kernel32, "SetFilePointer");
	if (wf == NULL) return;
	HANDLE h = CreateFileA (VP_DBG_LOG, GENERIC_WRITE, FILE_SHARE_READ, NULL,
	                        OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (h == (HANDLE)(int)-1) return;
	if (sfp != NULL) sfp (h, 0, NULL, 2); // FILE_END = 2
	DWORD written = 0;
	wf (h, msg, (DWORD)strlen(msg), &written, NULL);
	CloseHandle (h);
}

// Shuffles existing VP objects (placed by the engine's "auto place victory point locations" feature)
// to our randomly selected positions stored in is->pending_vp_tile_indices.
//
// Old VP positions are taken from map->Starting_Locations[] (where the engine auto-places VPs for
// random maps).  We read and clear Tile.VictoryPoint on those tiles, then stamp the new tiles.
// VP object coordinates (tile_x/tile_y in Victory_Point_Body) still need a separate update once we
// locate the VP list in memory — tracked in project memory.
static void
shuffle_victory_points (void)
{
	if ((is->pending_vp_count <= 0) || (is->pending_vp_tile_indices == NULL))
		return;

	Map * map        = &p_bic_data->Map;
	int   tile_count = map->TileCount;
	int   pending    = is->pending_vp_count;
	int * new_idxs   = is->pending_vp_tile_indices;
	int   civ_count  = map->Civ_Count;

	short * old_vals = (short *)malloc (pending * sizeof (short));
	int   * old_idxs = (int   *)malloc (pending * sizeof (int));
	if ((old_vals == NULL) || (old_idxs == NULL)) {
		free (old_vals);
		free (old_idxs);
		return;  // preserve pending data for retry
	}

	// --- Step 1: collect old VP tile positions from starting locations ---
	// The engine's "auto place VPs" puts one VP per starting location.
	// Starting_Locations is 1-indexed; index 0 is unused.
	int found = 0;
	for (int j = 1; (j <= civ_count) && (found < pending); j++) {
		int idx = map->Starting_Locations[j];
		if ((idx < 0) || (idx >= tile_count)) continue;
		Tile * t = tile_at_index (map, idx);
		if ((t == NULL) || (t == p_null_tile)) continue;
		old_vals[found] = t->VictoryPoint;
		old_idxs[found] = idx;
		found++;
	}

	// --- Diagnostic: write to log file ---
	{
		char buf[2048];
		int  pos = 0;
		...
		pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
			"=== VP shuffle_victory_points ===\r\n"
			"pending=%d  civ_count=%d  found=%d\r\n\r\n",
			pending, civ_count, found);

		// Starting location tiles: index, coords, VictoryPoint value before shuffle
		pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
			"Starting location tiles (tile fields BEFORE shuffle):\r\n"
			"  (VP=VictoryPoint  SP=SquareParts  OV=Overlays)\r\n");
		for (int j = 1; (j <= civ_count) && (j <= 32); j++) {
			int idx = map->Starting_Locations[j];
			int sx = -1, sy = -1;
			short vp = -9999; short sp = -9999; int ov = -9999;
			if ((idx >= 0) && (idx < tile_count)) {
				tile_index_to_coords (map, idx, &sx, &sy);
				Tile * t = tile_at_index (map, idx);
			...
			pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
				"  sl[%d] (%3d,%3d)  VP=%d  SP=0x%04X  OV=0x%08X\r\n",
				j, sx, sy, (int)vp, (unsigned)sp, (unsigned)ov);
		}

		// Sample non-starting tiles for comparison
		pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
			"\r\nSample non-starting tiles (first 4):\r\n");
		for (int i = 0; i < 4 && i < tile_count; i++) {
			int sx, sy;
			tile_index_to_coords (map, i, &sx, &sy);
			Tile * t = tile_at_index (map, i);
			short  vp = (t && t != p_null_tile) ? t->VictoryPoint : -9999;
			short  sp = (t && t != p_null_tile) ? t->SquareParts  : -9999;
			int    ov = (t && t != p_null_tile) ? t->Overlays      : -9999;
			pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
				"  idx=%-5d (%3d,%3d)  VP=%d  SP=0x%04X  OV=0x%08X\r\n",
				i, sx, sy, (int)vp, (unsigned)sp, (unsigned)ov);
		}

		// New target positions
		pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
			"\r\nNew target positions:\r\n");
		for (int k = 0; k < pending && k < 32; k++) {
			int nx, ny;
			tile_index_to_coords (map, new_idxs[k], &nx, &ny);
			pos += snprintf (buf + pos, (int)sizeof(buf) - pos,
				"  [%d] idx=%-5d -> (%3d,%3d)\r\n", k, new_idxs[k], nx, ny);
		}
	...
	if (found == 0) {
		// vp_dbg_log ("found=0, nothing to shuffle.\r\n\r\n");
		free (old_vals);
		free (old_idxs);
		return;  // preserve pending data for retry
	}
	...
	int updated = 0;
	// --- Scan memory for VLOC objects and update their tile_x/tile_y ---
	// Victory_Point layout: Base(28 bytes) + Victory_Point_Body(20 bytes)
	//   Base.ClassName  = "VLOC" (0x434F4C56) at offset 8  (ip[2])
	//   Body.tile_x     at offset 36 (ip[9])
	//   Body.tile_y     at offset 40 (ip[10])
	// For each hit, match tx/ty against known old starting-location coords.
	// If matched, update to the corresponding new target position.
	{
#define VLOC_CLASS  0x434F4C56
#define VP_OBJ_SIZE 48
		typedef struct {
			void * BaseAddress;
			void * AllocationBase;
		...
		typedef DWORD (WINAPI * VirtualQuery_t)(void *, MBI *, DWORD);
		VirtualQuery_t vq = (VirtualQuery_t)(*p_GetProcAddress)(is->kernel32, "VirtualQuery");

		int hits    = 0;
		char scan_buf[2048] = "";

		if (vq != NULL) {
			char * addr = (char *)0x10000;
			while (addr < (char *)0xBFFFFFFF) {
				MBI mbi;
				if (vq (addr, &mbi, (DWORD)sizeof(mbi)) == 0) break;
				if ((mbi.State == 0x1000) &&         // MEM_COMMIT
				    ((mbi.Protect & 0xFF) > 1) &&    // not PAGE_NOACCESS
				    ((mbi.Protect & 0x100) == 0)) {  // not PAGE_GUARD
					char * end = addr + mbi.RegionSize;
					for (char * p = addr; p + VP_OBJ_SIZE <= end; p += 4) {
						int * ip = (int *)p;
						if (ip[2] != VLOC_CLASS) continue;
						hits++;
						int tx = ip[9], ty = ip[10];
						// Match against old starting-location coords
						for (int j = 0; j < found; j++) {
							int ox, oy;
							tile_index_to_coords (map, old_idxs[j], &ox, &oy);
							if (tx == ox && ty == oy) {
								int nx, ny;
								tile_index_to_coords (map, new_idxs[j], &nx, &ny);
																								// Update VLOC Body.tile_x/tile_y (gameplay/scoring)
								ip[9]  = nx;
								ip[10] = ny;
								// Move VP index from old tile to new tile (rendering)
								Tile * old_t = tile_at_index (map, old_idxs[j]);
								Tile * new_t = tile_at_index (map, new_idxs[j]);
								short vp_idx = -1;
								if (old_t && old_t != p_null_tile) {
									vp_idx = ((short *)old_t)[17];
									((short *)old_t)[17] = -1;  // clear old tile VP marker
								}
								if (new_t && new_t != p_null_tile)
									((short *)new_t)[17] = vp_idx;  // stamp new tile VP marker
								char tmp[128];
								snprintf (tmp, sizeof tmp,
																																				"  VLOC@%p: (%d,%d)->(%d,%d) vp_idx=%d
",
									(void*)p, ox, oy, nx, ny, (int)vp_idx);
								int sl = strlen(scan_buf), tl = strlen(tmp);
								if (sl + tl < (int)sizeof(scan_buf) - 1)
									strcpy (scan_buf + sl, tmp);
								updated++;
								break;
							}
						}
		...
		snprintf (hdr, sizeof hdr,
			"VLOC scan: %d hit(s), %d updated\r\n", hits, updated);
		// vp_dbg_log (hdr);
		// if (updated > 0) vp_dbg_log (scan_buf);
		// vp_dbg_log ("\r\n");
#undef VLOC_CLASS
#undef VP_OBJ_SIZE
	}
	...
	if (updated == 0) return;  // no VLOCs yet; preserve pending data for retry
...
patch_Map_impl_generate (Map * this, int edx, int seed, bool is_multiplayer_game, int num_seafaring_civs)
{
	...
	// Select desired VP positions now, while the map seed is fresh.
	// The actual shuffle happens later in patch_Leader_begin_turn, after the engine has
	// auto-placed VP objects via its "auto place victory point locations" game setting.
	place_victory_points ();
	if (is->pending_vp_count > 0)
		is->vps_await_shuffle = true;
}
...
patch_Leader_begin_turn (Leader * this)
{
	...
	// Apply deferred VP shuffle for random map games. The engine auto-places VP objects sometime
	// between map generation and the first turn; by now they exist and can be shuffled.
	if (is->vps_await_shuffle)
		shuffle_victory_points ();

	// Diagnostic: log VLOC positions + spawn tile raw bytes at turn 0.
	if (*p_current_turn_no == 0 && this->ID == 1) {
		Map * map2 = &p_bic_data->Map;
		char lbuf[1024];
		int  lpos = 0;
		lpos += snprintf(lbuf+lpos, (int)sizeof(lbuf)-lpos,
			"=== Turn-0 state ===
");

		// Log raw tile bytes at spawn locations (to find which field the engine sets for VPs)
		for (int j = 1; j <= map2->Civ_Count && j <= 8; j++) {
			int idx = map2->Starting_Locations[j];
			Tile * t = (idx >= 0 && idx < map2->TileCount) ? tile_at_index(map2, idx) : NULL;
			if (t == NULL || t == p_null_tile) continue;
			short * sp = (short *)t;
			int x=-1, y=-1;
			tile_index_to_coords(map2, idx, &x, &y);
			lpos += snprintf(lbuf+lpos, (int)sizeof(lbuf)-lpos,
				"sl[%d] (%d,%d): s8=0x%04X s10=0x%04X s12=0x%04X s14=0x%04X s16=0x%04X s18=0x%04X s20=0x%04X s22=0x%04X s24=0x%04X s26=0x%04X s28=0x%04X s30=0x%04X s32=0x%04X s34=0x%04X
",
				j, x, y,
				(unsigned)(unsigned short)sp[4],  (unsigned)(unsigned short)sp[5],
				(unsigned)(unsigned short)sp[6],  (unsigned)(unsigned short)sp[7],
				(unsigned)(unsigned short)sp[8],  (unsigned)(unsigned short)sp[9],
				(unsigned)(unsigned short)sp[10], (unsigned)(unsigned short)sp[11],
				(unsigned)(unsigned short)sp[12], (unsigned)(unsigned short)sp[13],
				(unsigned)(unsigned short)sp[14], (unsigned)(unsigned short)sp[15],
				(unsigned)(unsigned short)sp[16], (unsigned)(unsigned short)sp[17]);
		}
		// Log 2 non-spawn tiles for comparison
		for (int i = 100; i <= 101; i++) {
			Tile * t = (i < map2->TileCount) ? tile_at_index(map2, i) : NULL;
			if (t == NULL || t == p_null_tile) continue;
			short * sp = (short *)t;
			int x=-1, y=-1;
			tile_index_to_coords(map2, i, &x, &y);
			lpos += snprintf(lbuf+lpos, (int)sizeof(lbuf)-lpos,
				"ns[%d] (%d,%d): s8=0x%04X s10=0x%04X s12=0x%04X s14=0x%04X s16=0x%04X s18=0x%04X s20=0x%04X s22=0x%04X s24=0x%04X s26=0x%04X s28=0x%04X s30=0x%04X s32=0x%04X s34=0x%04X
",
				i, x, y,
				(unsigned)(unsigned short)sp[4],  (unsigned)(unsigned short)sp[5],
				(unsigned)(unsigned short)sp[6],  (unsigned)(unsigned short)sp[7],
...
patch_Unit_can_disembark_anything (Unit * this, int edx, int tile_x, int tile_y)
{
	...
	if (base) {
		...
		FOR_UNITS_ON (uti, this_tile)
			if ((uti.unit->Body.Container_Unit == this->Body.ID) && is_below_stack_limit (target_tile, this->Body.CivID, uti.unit->Body.UnitTypeID)) {
...
patch_Tile_check_water_for_retreat_on_defense (Tile * this)
{
	...
	if ((! retreat_blocked) &&
	    (defender != NULL) &&
	    ! is_below_stack_limit (this, defender->Body.CivID, defender->Body.UnitTypeID))
		retreat_blocked = true;
...
patch_Leader_spawn_unit (Leader * this, int edx, int type_id, int tile_x, int tile_y, int barb_tribe_id, int id, bool param_6, LeaderKind leader_kind, int race_id)
{
	...
	if (is->current_config.enable_districts &&
	    is->current_config.air_units_use_aerodrome_districts_not_cities) {
		int aerodrome_id = AERODROME_DISTRICT_ID;
		if ((p_bic_data->UnitTypes[type_id].Unit_Class == UTC_Air) && (aerodrome_id >= 0)) {
...
patch_Map_place_scenario_things (Map * this)
{
	...
	// Shuffle VP locations that the engine just placed (from BIQ TILE data or auto-placement).
	// place_victory_points() selects our desired random positions; shuffle_victory_points() moves
	// the engine's VP objects there.
	place_victory_points ();
	shuffle_victory_points ();
...
patch_Unit_check_airdrop_target (Unit * this, int edx, int tile_x, int tile_y)
{
	return Unit_check_airdrop_target (this, __, tile_x, tile_y) &&
		is_below_stack_limit (tile_at (tile_x, tile_y), this->Body.CivID, this->Body.UnitTypeID) &&
...
patch_Unit_check_airlift_target (Unit * this, int edx, int tile_x, int tile_y)
{
	...
	return is_below_stack_limit (tile, this->Body.CivID, this->Body.UnitTypeID);
}
...
patch_City_count_airports_for_ai_airlift_target (City * this, int edx, enum ImprovementTypeFlags airport_flag)
{
	...
	if ((tr > 0) && ! is_below_stack_limit (tile_at (this->Body.X, this->Body.Y), this->Body.CivID, unit->Body.UnitTypeID))
		return 0;
...
{
	if (! is_below_stack_limit (tile_at (x, y), this->Body.CivID, this->Body.UnitTypeID)) {
...
patch_Unit_ai_go_to_capital (Unit * this)
{
	...
	if ((capital != NULL) &&
	    ! is_below_stack_limit (tile_at (capital->Body.X, capital->Body.Y), this->Body.CivID, this->Body.UnitTypeID))
		return false;
...
patch_Unit_is_in_rebase_range (Unit * this, int edx, int tile_x, int tile_y)
{
	...
	return in_range && is_below_stack_limit (tile_at (tile_x, tile_y), this->Body.CivID, this->Body.UnitTypeID);
}
...
patch_Unit_check_rebase_target (Unit * this, int edx, int tile_x, int tile_y)
{
	...
	// 6 is the game's standard value, so fall back on the base game logic in that case
	if (is->current_config.rebase_range_multiplier == 6) {
		return Unit_check_rebase_target (this, __, tile_x, tile_y) && is_below_stack_limit (tile_at (tile_x, tile_y), this->Body.CivID, this->Body.UnitTypeID);
